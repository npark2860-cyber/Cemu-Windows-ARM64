...
public:
        ...
        : wxDialog(parent, wxID_ANY, _("ARM64 Diagnostics"), wxDefaultPosition, wxSize(720, 760), wxDEFAULT_DIALOG_STYLE | wxRESIZE_BORDER)
    {
        ...
        preset->Append(_("Custom"));
        preset->Append(_("Minimal"));
        preset->Append(_("Performance"));
        preset->Append(_("Vulkan"));
        preset->Append(_("JIT"));
        preset->Append(_("Graphics"));
        preset->Append(_("Input"));
        ...
        hitch->AddStretchSpacer();
        hitch->Add(reset, 0, wxALL, 5);
        root->Add(hitch, 0, wxEXPAND);
        ...
        preset->Bind(wxEVT_CHOICE, [this,preset](wxCommandEvent&){
            const int p = preset->GetSelection();
            RuntimeDiagnostics::SetAll(false);
            if (p == 1)
            {
                for (auto f : {DiagFlag::FrameTiming,DiagFlag::QueueSubmit,DiagFlag::CpuWaitBreakdown,DiagFlag::HitchTrigger})
                    RuntimeDiagnostics::SetEnabled(f, true);
            }
            else if (p == 2)
            {
                for (auto f : {DiagFlag::FrameTiming,DiagFlag::LatteThreadTiming,DiagFlag::DrawCallCount,DiagFlag::PipelineCompileTime,DiagFlag::PerfPipelineCache,DiagFlag::BarrierCount,DiagFlag::RenderPassCount,DiagFlag::QueueSubmitCount,DiagFlag::UploadStallTiming,DiagFlag::PresentTiming,DiagFlag::GpuTimestamp,DiagFlag::CpuWaitBreakdown,DiagFlag::DescriptorStats,DiagFlag::MemoryUploadStats,DiagFlag::JitPerformance,DiagFlag::HitchTrigger,DiagFlag::DiagnosticOverhead})
                    RuntimeDiagnostics::SetEnabled(f, true);
            }
            else if (p == 3)
            {
                for (auto f : {DiagFlag::CommandBufferLifecycle,DiagFlag::QueueSubmit,DiagFlag::FenceLifecycle,DiagFlag::SemaphoreFlow,DiagFlag::SubmitCompletion,DiagFlag::DeviceLostSubmitError,DiagFlag::PipelineCache,DiagFlag::PipelineCreation,DiagFlag::PipelineFailure,DiagFlag::PipelineStateSnapshot,DiagFlag::RenderPassBeginEnd,DiagFlag::PipelineBarriers,DiagFlag::SynchronizationSummary,DiagFlag::GpuTimestamp,DiagFlag::CpuWaitBreakdown,DiagFlag::DescriptorStats})
                    RuntimeDiagnostics::SetEnabled(f, true);
            }
            else if (p == 4)
            {
                for (auto f : {DiagFlag::JitBlockLifecycle,DiagFlag::GuestHostMapping,DiagFlag::BranchPatching,DiagFlag::ReadyReICache,DiagFlag::JitExecutionEntry,DiagFlag::Arm64ExceptionContext,DiagFlag::GuestMemoryAccess,DiagFlag::JitPerformance})
                    RuntimeDiagnostics::SetEnabled(f, true);
            }
            else if (p == 5)
            {
                for (auto f : {DiagFlag::PipelineCache,DiagFlag::PipelineCreation,DiagFlag::PipelineFailure,DiagFlag::ShaderCreation,DiagFlag::ShaderVS,DiagFlag::ShaderPS,DiagFlag::ShaderGS,DiagFlag::ShaderAuxHash,DiagFlag::ShaderInterface,DiagFlag::GLSLCompileFailure,DiagFlag::SPIRVCompileFailure,DiagFlag::DumpFailedShader,DiagFlag::RenderPassBeginEnd,DiagFlag::FBOChanges,DiagFlag::AttachmentUsage,DiagFlag::PipelineBarriers,DiagFlag::FeedbackUse,DiagFlag::ImageLayoutTransition,DiagFlag::TextureCache})
                    RuntimeDiagnostics::SetEnabled(f, true);
            }
            else if (p == 6)
            {
                for (auto f : {DiagFlag::VPAD,DiagFlag::KPAD,DiagFlag::ControllerSlot,DiagFlag::PlayerIndex,DiagFlag::ChannelMapping,DiagFlag::ConnectDisconnect,DiagFlag::InputReadSummary})
                    RuntimeDiagnostics::SetEnabled(f, true);
            }
            else if (p == 7)
            {
...
private:
    void RefreshChecks()
    {
        for (auto& entry : m_boxes)
            entry.first->SetValue(RuntimeDiagnostics::Enabled(entry.second));
        m_master->SetValue(RuntimeDiagnostics::AnyEnabled());
    }
...
print("[lean-diagnostics-ui] installed; unsupported flags are omitted from the UI")
