...
\t}
"""
branch_new = """\tif (RuntimeDiagnostics::Enabled(RuntimeDiagnostics::Flag::BranchPatching))
...
"""
t = replace_once(t, branch_block, branch_new, "branch patch diagnostics")
...
""",
    "guest-host mapping diagnostics",
)
backend.write_text(t, encoding="utf-8", newline="\n")

recompiler = Path("src/Cafe/HW/Espresso/Recompiler/PPCRecompiler.cpp")
t = recompiler.read_text(encoding="utf-8")
t = ensure_include(
    t,
...
\t\tif (n <= 100 || (n % 10000ULL) == 0)
\t\t\tcemuLog_log(LogType::Force, "[ARM64_JIT] ENTER n={} guestPC=0x{:08x} host={}", n, hCPU->instructionPointer, (void*)funcPtr);
\t}
#endif
""",
    "JIT execution-entry diagnostics",
)
recompiler.write_text(t, encoding="utf-8", newline="\n")

exc = Path("src/Common/ExceptionHandler/ExceptionHandler_win32.cpp")
t = exc.read_text(encoding="utf-8")
t = ensure_include(
    t,
...
exc.write_text(t, encoding="utf-8", newline="\n")
...
print("[lean-arm64] JIT, mapping, branch, readyRE, entry and crash diagnostics installed")
