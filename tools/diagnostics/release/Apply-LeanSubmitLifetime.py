...
text = replace_once(
    text,
    reset_cmd,
    reset_cmd + """\tif (RuntimeDiagnostics::Enabled(RuntimeDiagnostics::Flag::CommandBufferLifecycle))
\t\tcemuLog_log(LogType::Force, "[CMD_LIFECYCLE] RESET slot={}", m_commandBufferIndex);
""",
    "command-buffer reset hook",
)
...
text = replace_count(
    text,
    reset_fence,
    reset_fence + """\tif (RuntimeDiagnostics::Enabled(RuntimeDiagnostics::Flag::FenceLifecycle))
...
fence_status_line = "\t\tVkResult fenceStatus = vkGetFenceStatus(m_logicalDevice, m_cmdBufferFences[m_commandBufferSyncIndex]);\n"
text = replace_once(
    text,
    fence_status_line,
    fence_status_line + """\t\tif (fenceStatus == VK_SUCCESS && RuntimeDiagnostics::Enabled(RuntimeDiagnostics::Flag::FenceLifecycle))
\t\t\tcemuLog_log(LogType::Force, "[FENCE_LIFECYCLE] SIGNALED slot={}", m_commandBufferSyncIndex);
\t\tif (fenceStatus == VK_SUCCESS && RuntimeDiagnostics::Enabled(RuntimeDiagnostics::Flag::SubmitCompletion))
\t\t\tcemuLog_log(LogType::Force, "[SUBMIT_COMPLETE] slot={} finishedCount={}", m_commandBufferSyncIndex, m_countCommandBufferFinished);
""",
    "fence status hooks",
)

unexpected = '\t\tUnrecoverableError(fmt::format("vkGetFenceStatus returned unexpected error {}", (sint32)fenceStatus).c_str());\n'
text = replace_once(
    text,
    unexpected,
...
text = replace_once(
    text,
    wait_line,
    wait_line + """\tif (RuntimeDiagnostics::Enabled(RuntimeDiagnostics::Flag::FenceLifecycle))
...
text = replace_once(
    text,
    submit_line,
    """\tif (RuntimeDiagnostics::Enabled(RuntimeDiagnostics::Flag::SemaphoreFlow))
\t{
...
""",
    "queue submit lifetime hooks",
)

path.write_text(text, encoding="utf-8", newline="\n")
print("[lean-submit] command/fence/semaphore/submit diagnostics installed")
