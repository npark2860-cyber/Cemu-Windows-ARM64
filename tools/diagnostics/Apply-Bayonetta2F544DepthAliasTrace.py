...
# ---------------------------------------------------------------------------
# LatteTexture.cpp: relation gap, creation and normal render-target writes.
# ---------------------------------------------------------------------------
...
// Bayonetta 2 JP f5442800 observation only. No cache behavior changes.
...
static size_t Bayo2F544SameAddressOverlapCount(LatteTexture* texture)
{
...
\t\t\t\t\t\t\t}
''',
    "zero-offset incompatible relation observation",
)
...
\t\tLatteTC_LookupTexturesByPhysAddr(newTexture->physAddress, aliases);
...
\t\t\taliases.size(), newTexture->list_compatibleRelations.size(), Bayo2F544SameAddressOverlapCount(newTexture),
\t\t\tinfo->lastDynamicUpdate, newTexture->isUpdatedOnGPU ? 1 : 0, newTexture->reloadFromDynamicTextures ? 1 : 0);
\t}
\tLatteTexture_UpdateTextureFromDynamicChanges(view->baseTexture);
''',
    "new f544 mapping observation",
)
...
''',
    "draw GPU write observation",
)
tex_path.write_text(tex, encoding="utf-8", newline="\n")


# ---------------------------------------------------------------------------
# LatteRenderTarget.cpp: representation switches/staleness and depth clears.
# ---------------------------------------------------------------------------
...
static uint64 s_bayo2F544ClearRequestCount = 0;
static uint32 s_bayo2F544LastClearRequestPitch = 0;
...
\t\t\t\t\t\t{
\t\t\t\t\t\t\tif (!alias || alias == current || !alias->isDepth || alias->format != current->format || !alias->sliceMipInfo)
\t\t\t\t\t\t\t\tcontinue;
...
\t\t\t\t\t\t\tnewestOtherEvent, newestOther ? newestOther->width : 0, newestOther ? newestOther->height : 0, newestOther ? newestOther->pitch : 0,
...
\t\t{
\t\t\tconst uint64 n = ++s_bayo2F544ClearRequestCount;
\t\t\tconst bool switched = s_bayo2F544LastClearRequestPitch != (uint32)depthBufferPitch;
\t\t\ts_bayo2F544LastClearRequestPitch = depthBufferPitch;
\t\t\tif (switched || n <= 256 || (n % 1000ULL) == 0)
\t\t\t\tcemuLog_log(LogType::Force,
\t\t\t\t\t"[BAYO2_F544_CLEAR_REQ] n={} switch={} event={} req={}x{} pitch={} tile={} fmt=0x{:x} depthClear={} stencilClear={} depthValue={} stencilValue={} candidates={}",
\t\t\t\t\tn, switched ? 1 : 0, eventCounter, depthBufferWidth, depthBufferHeight, depthBufferPitch, (uint32)depthBufferTileMode, (uint32)depthBufferFormat,
\t\t\t\t\thasDepthClear ? 1 : 0, hasStencilClear ? 1 : 0, clearDepth, clearStencil, list_depthClearTextures.size());
\t\t}
\t\tbool foundMatchingDepthBuffer = false;
''',
    "f544 clear request observation",
)
...
\t\t\tLatteRenderTarget_applyTextureDepthClear(texItr, depthBufferViewFirstSlice, depthBufferMipIndex, hasDepthClear, hasStencilClear, clearDepth, clearStencil, eventCounter);
''',
    "f544 clear target observation",
)
rt_path.write_text(rt, encoding="utf-8", newline="\n")


# Static safety checks. The diagnostic must not repair or redirect the gap.
...
# There is exactly one original TrackDataOverlap call, in the non-zero-offset
# branch. If this count changes, the observation script has accidentally become
# a behavior fix.
if patched_tex.count("LatteTexture_TrackDataOverlap(texture, sliceMipInfo, occupancy);") != 1:
    raise RuntimeError("TrackDataOverlap behavior changed")
