...
All other historical branches are **read-only reference/archive branches** and are not valid targets for new work unless the user explicitly requests historical inspection. `main` is outside this workflow and must not be touched except for these canonical documentation entrypoints.
...
- Do not modify `main` except for canonical branch-policy / active-role documentation entrypoints.
